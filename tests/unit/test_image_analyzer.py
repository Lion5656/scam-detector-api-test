...
class _FakeOnlinePriceService:
    ...
    def estimate_price(
        ...
        *,
        ...
        search_tool = "serp_api" if self.value > 0 else "unused"
        return self.value, search_tool
...
狀況 二手・近全新""",
    ...
    assert result.search_tool == "serp_api"
    assert identifier.received == "iphone 15 256GB"
    assert decision.received["selling_price"] == 25000
    assert decision.received["market_price_source"] == "online"
    assert "has_risk" not in decision.received


def test_successful_analysis_records_case(monkeypatch):
    monkeypatch.setattr(
        "backend.services.image_price_service.case_recorder.settings.CASE_MEMORY_ENABLED",
        True,
    )
...
def test_detector_marks_unknown_when_online_search_results_are_insufficient(
    monkeypatch,
):
    monkeypatch.setattr(
        "backend.services.image_price_service.case_recorder.settings.CASE_MEMORY_ENABLED",
