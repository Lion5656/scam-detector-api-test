...
# 建立關鍵字比對規則
MONEY_RULE = re.compile(r"\$|\bUSD\b|TWD|NT|錢|元|金額|台幣|美金|人民幣|日圓|匯款|匯入|佣金|手續費|未繳清|未繳費|交易連結|交易鏈結|入帳|出金|付費|付款|付現|繳費|刷卡|退款|退費|轉帳")
...
RULES: List[Tuple[re.Pattern, int, str]] = [
    ...
    (URGENCY_RULE, 45, "有催促及急促語氣"),
    (INVEST_RULE, 45, "投資及獲利異常相關誘導字詞"),
    (IMPERSONATE_RULE, 50, "假冒官方/客服及操作異常相關誘導用詞"),
    (SENSITIVE_RULE, 50, "索取個資及敏感資訊等相關用詞"),
    (GIFT_RULE, 30, "免費貼圖/贈品等相關誘導用詞"),
...
def compare_rules(text: str) -> Tuple[int, str, List[str]]:
    ...
    for pattern, point, sentence in RULES:
        ...
        if is_debunk:
            return 20, "，含反詐騙宣導用詞", []
        if results:
            score += len(results) * point # 計算命中/次的分數
            reason_list.append(sentence)
            hit_list.extend(results)
    
    reason = "含" + ",".join(reason_list) if reason_list is not None else ""
